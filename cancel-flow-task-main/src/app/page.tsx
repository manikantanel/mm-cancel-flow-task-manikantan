...
import CancelDoneModal from '@/components/CancelDoneModal';
...
// a subscription *id*, not user_id  (replace with a real UUID from your DB/seed)
const subscriptionId = '1870f93a-f5f3-4246-8a4e-cf84536b4e0c';
          ...
          <div className="px-6 py-8 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-indigo-50">
            <div className="flex items-center justify-between">
              ...
              <div className="flex space-x-3">
                <button
                  onClick={handleClose}
                  className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-[#8952fc] rounded-lg hover:bg-[#7b40fc]"
                >
                  {/* Back icon */}
          ...
          <div className="px-6 py-6">
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showAdvancedSettings ? 'max-h-[800px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
              <div className="space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {/* Cancel button → bootstrap then open chooser */}
                <button
                  onClick={async () => {
                    ...
                    if (r.ok) {
                      ...
                      setVariant(j.variant as Variant); // must be set before chooser
                      setPriceCents(j.priceCents);
                    } else {
                      console.error('bootstrap failed', r.status, await r.text().catch(() => ''));
                    }
                    setShowCancel(true);
                    setStage('cancel');
                  }}
                  className="inline-flex items-center justify-center w-full px-4 py-3 bg-white border border-red-200 text-red-600 rounded-lg hover:bg-red-50 hover:border-red-300 transition-all shadow-sm group"
                >
                ...
                <CancelModal
                  ...
                  onStillLooking={() => {
                    setShowCancel(false);
                    // branch by server-chosen variant (fallback to B if unknown)
                    setStage(variant === 'A' ? 'usage' : 'downsell');
                  }}
                />
                ...
                {/* Usage survey (A direct or B after decline) */}
                <UsageSurveyModal
                  open={stage === 'usage'}
                  onBack={() => setStage(variant === 'B' ? 'downsell' : 'cancel')}
                  onClose={() => setStage('idle')}
                  onContinue={() => setStage('reasonsB')}
                  fullPrice={priceCents / 100}
                  showOffer={true}
